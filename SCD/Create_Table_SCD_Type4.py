...
try:
    ...
    postgres_cursor.execute("CREATE TABLE IF NOT EXISTS s4_nnl.customer_scd_type4 (cust_id VARCHAR(255) PRIMARY KEY , cust_nm VARCHAR(255), birth_date DATE, add_id VARCHAR(255), opn_dt DATE, end_dt DATE, record_time DATE)")
    ...
    def process_data(query, cnt):
        ...
        extras.execute_values(
            postgres_cursor,
            """
            INSERT INTO s4_nnl.customer_scd_type4 (cust_id, cust_nm, birth_date, add_id, opn_dt, end_dt, record_time)
            VALUES %s
